...
def detect_flower(out1, flower_name): # detecting flower type if it is between defined values
    ...
    if  -1 < out1 < 0.7: 
        rospy.loginfo(f"flower is Iris-setosa, real answer: {flower_name}")
    elif 0.7 < out1 < 1.6:
        rospy.loginfo(f"flower is Iris-versicolor, real answer: {flower_name}")
    elif  1.6 < out1 < 5:  
        rospy.loginfo(f"flower is Iris-virginica, real answer: {flower_name}")
    else:
        rospy.loginfo(f"cannot detect, real answer: {flower_name}")
